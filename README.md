...
- `LoginType`：登录方式，`direct` 为旧版树维直登，`authserver` 为统一身份认证
- `AuthServerURL`：统一身份认证登录地址，`LoginType` 为 `authserver` 时填写
...
- 感谢原 [getMyCourses](https://github.com/whoisnian/getMyCourses) / getMyCourse 项目提供的基础思路。
