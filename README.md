...
- `LoginType`：默认登录方式，`direct` 为旧版树维直登，`authserver` 为统一身份认证
- `AuthServerURL`：默认统一身份认证登录地址。authserver 学校应在配置中填写一次，业务请求不需要重复传递
...
登录方式由 `config.json` 提供默认值。正常使用时只在 `login` 请求中传一次账号密码；同一条 WebSocket 连接里的后续业务请求只传 `Type` 和业务参数，不需要重复传账号密码，也不需要重复传 `LoginType` / `AuthServerURL`。

WebSocket 是长连接。Go 与 Python 服务会在同一条连接内记住最近一次登录请求里的登录凭据与登录参数，因此常规使用可以先发一次 `login`，后续同连接请求只传 `Type` 和业务参数。
...
教师账号可调用该接口读取 `courseTableForTeacher.action` 并解析教师课表。
...
返回结构与学生 `allcourse` 一致，`Data` 为课程数组。
...
然后再在 `teacherexam` 请求中传 `ExamBatchID` 覆盖默认批次。主考与监考字段会分别返回为 `ChiefExaminer` 和 `Invigilators`；如果教务页面当前批次没有下发这些列或该分类暂无数据，字段会保留为空字符串。
...
```json
{
	...
	"Data": [
		{
			"Category": "我的教学任务",
			"CourseID": "22101301.16",
			"CourseName": "工程化学",
			"Department": "化学与环境科学学院",
			"Credit": "2",
			"StudentCount": "56",
...
该接口走树维公共页面 `publicFree!search.action`，不区分学生或教师账号，也不要求登录。默认按小节查询，`RoomTimeType` 为 `0` 时 `TimeBegin` / `TimeEnd` 表示小节；为 `1` 时表示 `HH:mm` 时间。

```json
{
	"Type": "freeroom",
	"DateBegin": "2026-06-05",
	"DateEnd": "2026-06-05",
	"RoomTimeType": "0",
	"TimeBegin": "1",
	"TimeEnd": "2"
